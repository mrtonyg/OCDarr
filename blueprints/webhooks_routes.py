...
@bp.route('/jellyfin-webhook', methods=['POST'])
def handle_jellyfin_webhook():
    """Handle webhooks from Jellyfin for playback progress."""
    shared.app.logger.info("Received webhook from Jellyfin")
    data = request.json
    if not data:
        return jsonify({'status': 'error', 'message': 'No data received'}), 400
    ...
    try:
        if data.get('NotificationType') == 'PlaybackProgress':
            ...
            if total_ticks > 0:
                ...
                shared.app.logger.info(f"Jellyfin playback progress: {progress_percent:.2f}%")
                ...
                if 45 <= progress_percent <= 55:
                    ...
                    if item_type == 'Episode':
                        ...
                        if all([series_name, season is not None, episode is not None]):
                            shared.app.logger.info(f"Processing Jellyfin episode: {series_name} S{season}E{episode}")
                            ...
                            if result.stderr:
                                shared.app.logger.error(f"Errors from servertosonarr.py: {result.stderr}")
                        else:
                            shared.app.logger.warning(f"Missing episode info: Series={series_name}, Season={season}, Episode={episode}")
                    else:
                        shared.app.logger.info(f"Item type '{item_type}' is not an episode, ignoring")
                else:
                    shared.app.logger.debug(f"Progress {progress_percent:.2f}% outside trigger range (45-55%), ignoring")
            else:
                shared.app.logger.warning("Total ticks is zero, cannot calculate progress")
        else:
            notification_type = data.get('NotificationType', 'Unknown')
            shared.app.logger.info(f"Jellyfin notification type '{notification_type}' is not PlaybackProgress, ignoring")

        return jsonify({'status': 'success'}), 200

    except Exception as e:
        shared.app.logger.error(f"Failed to process Jellyfin webhook: {str(e)}", exc_info=True)
        return jsonify({'status': 'error', 'message': str(e)}), 500
